    ...
    return (        <div className="min-h-screen pt-24 md:pt-28 pb-12 px-6 sm:px-10 md:px-16">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center lg:items-start">
                    ...
                    >
                        ...
                        <p className="text-sm md:text-lg text-slate-600 leading-relaxed mb-6">
                            Mapzest is a premier geospatial platform backed by a team of expert Earth Observation scientists. As a specialized product of <span className="font-semibold text-blue-900">TerrAqua UAV</span>, we leverage a robust foundation of high-resolution data acquisition and processing.
                        </p>
                        <p className="text-sm md:text-lg text-slate-600 leading-relaxed mb-6">
                            <span className="font-semibold text-slate-800">How we work:</span> <span className="font-semibold text-blue-900">TerrAqua UAV</span> captures and processes cutting-edge drone and satellite imagery. Mapzest then transforms this analysis-ready data into comprehensive, user-centric platforms tailored for diverse industrial needs, specific requirements, and strategic provisions.
                        </p>
                    </motion.div>
                </div>
            </section>
            ...
            <section className="max-w-7xl mx-auto mb-10 sm:mb-12">
                <div className="text-center mb-6 md:mb-8">
                    ...
                    <p className="text-slate-650 max-w-2xl mx-auto text-sm md:text-lg font-normal">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
                    {values.map((item, i) => (
                        <div key={i} className="group h-full">
                            <div className="bg-white/95 border border-slate-200 h-full p-5 sm:p-8 rounded-2xl sm:rounded-[30px] overflow-hidden flex flex-col justify-start relative shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-md hover:border-slate-300">
                                ...
                                <div className="relative mb-3 sm:mb-6 flex items-center justify-start w-fit">
                                    <div className={`absolute inset-0 rounded-full border border-dashed ${item.borderColor} animate-spin-slow scale-110 sm:scale-125 transition-all duration-500`}></div>
                                    ...
                                    <div className={`relative p-2.5 sm:p-3 bg-white border border-slate-200/60 rounded-full w-fit ${item.hoverBg} group-hover:scale-110 ${item.hoverBorder} transition-all duration-500 flex items-center justify-center shadow-md z-10`}>
                                        <item.Icon strokeWidth={1.6} fill="currentColor" fillOpacity={0.12} className={`w-5.5 h-5.5 sm:w-7 sm:h-7 ${item.color} transition-colors duration-300`} />
                                    </div>
                                </div>
 
                                <h3 className="text-base sm:text-xl font-bold mb-1.5 sm:mb-3 text-slate-900 transition-colors">{item.title}</h3>
